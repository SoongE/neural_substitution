...
class DyRep(object):
    ...
    def _reset_optimizer(self):
        param_groups = get_params(self.model, lr=0.1, weight_decay=1e-5, filter_bias_and_bn=self.filter_bias_and_bn,
                                  sort_params=True)
    ...
    def adjust_model(self):
        ...
        for k, m in self.model.named_modules():
            if isinstance(m, self.dbb_block_fn):
    ...
    def load_dbb_graph(self, dbb_list: list):
        ...
        print(dbb_list)
        assert not any(
            [isinstance(m, self.dbb_block_fn)
        ...
        for key, branches in dbb_list:
            ...
            dbb_m = self.dbb_block_fn(conv_m.in_channels,
                                      conv_m.out_channels,
