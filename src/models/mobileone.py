...
class MobileOneBlock(nn.Module):
    ...
    def _get_kernel_bias(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        bias_conv = 0
        for ix in range(self.num_conv_branches):
            _kernel, _bias = self._fuse_bn_tensor(self.rbr_conv[ix], self.n_flow)
            kernel_conv += _kernel
            bias_conv += _bias
...
@register_model
def mobileone(name, num_classes: int = 1000, inference_mode: bool = False,
...
if __name__ == '__main__':
    ...
    model = mobileone('', num_classes=100, variant="s0+", substitution=True, stem_type='cifar')
    print(model(x).shape)