...
from timm.layers import ConvMlp, LayerNorm, ClassifierHead, NormMlpClassifierHead
...
class MaxxVitBlock(nn.Module):
    ...
    def __init__(
        ...
        conv_cls = ConvNeXtBlock if conv_cfg.block_type == 'convnext' else block_fn
...
backbones = {
    'maxTiny': dict(variant='maxvit_tiny_tf_224', cfg_variant='maxvit_tiny_tf'),
    'maxSmall': dict(variant='maxvit_small_tf_224', cfg_variant='maxvit_small_tf'),
    'maxBase': dict(variant='maxvit_base_tf_224', cfg_variant='maxvit_base_tf'),
}
...
def SubMaxViT(name, pretrained=False, **kwargs):
    name = name.split('_')
    if len(name) == 1:
        b, m = name[0], 'origin'
    else:
...
if __name__ == '__main__':
    ...
    model_org = SubMaxViT('maxTiny_origin')

    parameter = count_parameters(model)
    parameter_org = count_parameters(model_org)
